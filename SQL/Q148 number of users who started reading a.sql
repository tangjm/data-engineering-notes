...
FROM
	...
	LEFT JOIN web_analytics.pageviews g ON w.visitor_id = g.visitor_id
	...
	LEFT JOIN web_analytics.pageviews l ON w.visitor_id = l.visitor_id
	AND l.url LIKE '%/library'
	AND l.referer_url LIKE '%/select-genres'
	AND l.created_at BETWEEN w.created_at
	AND w.created_at + '30 minutes' :: interval
	LEFT JOIN web_analytics.pageviews r ON w.visitor_id = r.visitor_id
